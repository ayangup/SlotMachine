...
def get_slotmachine_spin(rows, cols, symbols):
    ...
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:] #this symbol gives the copy, so the current_symbols does not contain everything in all_symbols.
        for _ in range(rows):
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"You do not have enough to bet this amount, current balance is : ${balance}")
    ...
    print(f"You won on lines:", *winnings_lines) #splat/unpack operator, it passes every single line from winning_lines list to its corresponding print function.
